...
function generatePage(post) {
  ...
  const readTime = Math.max(1, Math.round(String(post.content).replace(/<[^>]*>/g, ' ').split(/\s+/).filter(Boolean).length / 200));
...
<head>
  ...
  <style>
    ...
    .blog-post-body h2[id] { scroll-margin-top: 96px; }
    :root { --read-max: 720px; }
    /* ── Slim reading-progress bar ── */
    .read-progress { position:fixed; top:0; left:0; height:3px; width:0; background:#00DF81; z-index:1200; transition:width 0.08s linear; }
    .blog-post-body { font-size:1.18rem; line-height:1.8; color:#2b2b2b; font-family:Inter,sans-serif; }
    .blog-post-body > p:first-of-type { font-size:1.32rem; line-height:1.65; color:#151515; margin-bottom:1.4em; }
    .blog-post-body h1,.blog-post-body h2,.blog-post-body h3,.blog-post-body h4 { font-family:Inter,sans-serif; font-weight:700; color:#0a0a0a; line-height:1.25; letter-spacing:-0.4px; margin-top:2em; margin-bottom:0.5em; }
    .blog-post-body h1 { font-size:2rem; }
    .blog-post-body h2 { font-size:1.6rem; }
    .blog-post-body h3 { font-size:1.28rem; }
    .blog-post-body h4 { font-size:1.08rem; }
    .blog-post-body p  { margin-bottom:1.5em; }
    .blog-post-body img { width:100%; border-radius:8px; margin:2em 0; display:block; }
    .blog-post-body blockquote { border-left:3px solid #00DF81; padding:4px 0 4px 22px; margin:2em 0; color:rgba(0,0,0,0.6); font-style:italic; font-size:1.22rem; }
    .blog-post-body ul,.blog-post-body ol { padding-left:1.4em; margin-bottom:1.5em; }
    .blog-post-body li { margin-bottom:0.7em; padding-left:0.2em; }
    .blog-post-body strong { color:#0a0a0a; font-weight:600; }
    .blog-post-body a { color:#0a7d4f; text-decoration:underline; text-underline-offset:3px; text-decoration-thickness:1px; transition:color 0.15s; }
    .blog-post-body a:hover { color:#00DF81; }
    .blog-post-body hr,.blog-post-body .post-hr { border:none; border-top:1px solid rgba(0,0,0,0.1); margin:2.6em 0; }
    .blog-post-body table { width:100%; border-collapse:collapse; margin-bottom:1.5em; font-size:0.98rem; }
    ...
    .post-grid { max-width:var(--container-max); margin:0 auto; padding:0 var(--container-padding); display:grid; grid-template-columns:250px 1fr; gap:64px; align-items:start; }
    .post-toc-wrap { position:sticky; top:110px; align-self:start; }
    .post-toc { display:flex; flex-direction:column; gap:28px; }
    .post-toc-title { font-size:1.05rem; font-weight:800; color:#0a0a0a; margin:0 0 16px; font-family:Inter,sans-serif; }
    .post-toc-list { list-style:none; padding:0; margin:0; border-left:2px solid rgba(0,0,0,0.09); }
    .post-toc-list a { display:block; padding:8px 0 8px 18px; margin-left:-2px; border-left:2px solid transparent; font-size:0.9rem; line-height:1.45; color:rgba(0,0,0,0.5); text-decoration:none; transition:color 0.18s,border-color 0.18s; }
    .post-toc-list a:hover { color:#0a0a0a; }
    .post-toc-list a.active { color:#0a0a0a; font-weight:600; border-left-color:#00DF81; }
    .post-toc-cta { background:#0a0a0a; border-radius:14px; padding:22px; }
    .post-toc-cta h4 { font-family:Inter,sans-serif; font-size:1rem; font-weight:700; color:#fff; margin:0 0 14px; line-height:1.35; }
    .post-toc-cta a { display:inline-flex; align-items:center; justify-content:space-between; gap:10px; width:100%; box-sizing:border-box; background:#00DF81; color:#000; font-family:Inter,sans-serif; font-size:0.85rem; font-weight:600; padding:12px 16px; border-radius:8px; text-decoration:none; transition:opacity 0.2s; }
    .post-toc-cta a:hover { opacity:0.9; }
    .post-share-label { font-size:0.75rem; font-weight:600; letter-spacing:0.5px; color:rgba(0,0,0,0.5); margin:0 0 12px; }
    .post-share-row { display:flex; gap:10px; }
    .post-share-btn { width:36px; height:36px; border-radius:8px; background:#0a0a0a; color:#fff; display:inline-flex; align-items:center; justify-content:center; text-decoration:none; transition:opacity 0.2s; }
    ...
    @media (max-width:768px) {
      .blog-post-body { font-size:1.08rem; }
      .blog-post-body > p:first-of-type { font-size:1.18rem; }
...
<nav class="navbar" id="navbar">
  <div class="nav-container">
    ...
    <div class="nav-links" id="navLinks">
      ...
      <a href="/services" class="nav-link">Service <span class="nav-slash"></span></a>
...
<main style="background:#ffffff;min-height:100vh;">
  ...
  <section style="padding-top:110px;padding-bottom:100px;">
    <div class="post-grid">

      <!-- ── Left: sticky Table of Contents + CTA + Share ── -->
      <aside class="post-toc-wrap">
        <div class="post-toc">
          ...
          <div class="post-toc-cta">
            <h4>Partner with our AI team.</h4>
            <a href="/contact">CONTACT US <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
          </div>

          <div>
            <p class="post-share-label">Share this article on:</p>
            <div class="post-share-row">
              <a class="post-share-btn" href="https://www.facebook.com/sharer/sharer.php?u=${postUrl}" target="_blank" rel="noopener" aria-label="Share on Facebook"><i class="fab fa-facebook-f"></i></a>
              <a class="post-share-btn" href="https://twitter.com/intent/tweet?url=${postUrl}&text=${shareText}" target="_blank" rel="noopener" aria-label="Share on X"><i class="fa-brands fa-x-twitter"></i></a>
              <a class="post-share-btn" href="https://www.linkedin.com/sharing/share-offsite/?url=${postUrl}" target="_blank" rel="noopener" aria-label="Share on LinkedIn"><i class="fab fa-linkedin-in"></i></a>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div>
        <a href="/blog" style="display:inline-flex;align-items:center;gap:6px;font-size:0.85rem;color:rgba(0,0,0,0.4);text-decoration:none;margin-bottom:28px;letter-spacing:0.1px;transition:color 0.2s;" onmouseover="this.style.color='#0a0a0a'" onmouseout="this.style.color='rgba(0,0,0,0.4)'">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
          Back to Blog
        </a>

        ${post.image ? `<img src="${post.image}" alt="${esc(post.title)}" style="width:100%;max-height:420px;height:auto;object-fit:cover;object-position:center;border-radius:10px;margin-bottom:32px;background:#f5f5f3;">` : ''}

        <p style="font-size:0.72rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#0a7d4f;margin-bottom:16px;">Field Notes</p>
        <h1 style="font-family:Inter,sans-serif;font-size:clamp(1.9rem,4vw,2.7rem);font-weight:800;line-height:1.15;color:#0a0a0a;letter-spacing:-0.6px;margin-bottom:20px;">${esc(post.title)}</h1>

        <div style="display:flex;align-items:center;gap:14px;font-size:0.85rem;color:rgba(0,0,0,0.5);flex-wrap:wrap;padding-bottom:22px;margin-bottom:8px;border-bottom:1px solid rgba(0,0,0,0.08);">
          <span style="display:inline-flex;align-items:center;gap:7px;">
            <span style="width:26px;height:26px;border-radius:50%;background:#0a0a0a;color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:0.75rem;font-weight:700;">${avatar}</span>
            Posted by: <strong style="color:#0a0a0a;font-weight:600;">${esc(post.author)}</strong>
          </span>
          <span style="width:3px;height:3px;border-radius:50%;background:rgba(0,0,0,0.3);"></span>
          <span>${dateStr} / ${readTime} min read${pdfMetaLink}</span>
        </div>
    ...
    <div class="footer-content">
      ...
      <div class="footer-right">
        <div class="footer-links-wrapper">
          ...
          <div class="footer-link-col"><h4 class="footer-col-title">Pages</h4><a href="/">Home</a><a href="/labs">Labs</a><a href="/services">Services</a></div>
          <div class="footer-link-col"><h4 class="footer-col-title">Company</h4><a href="/about">About</a><a href="/contact">Contact Us</a><a href="/legal/privacy">Privacy Policy</a><a href="/legal/terms">Terms of Service</a><a href="/legal/cookies">Cookie Policy</a></div>
        </div>
...
<script>
  (function () {
    ...
    var heads = body ? Array.prototype.slice.call(body.querySelectorAll('h2')) : [];
    var items = [];
    ...
    function update() {
      if (bar && article) {
        ...
        var scrolled = window.scrollY - top + 200;
        bar.style.width = Math.min(Math.max(scrolled / total, 0), 1) * 100 + '%';
      }
      if (items.length) {
        var cur = items[0];
        items.forEach(function (it) { if (it.el.getBoundingClientRect().top <= 120) cur = it; });
